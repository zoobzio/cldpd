...
DiscoverPod("~/.cldpd/pods", "myrepo")
  ...
  +-- Resolve InheritEnv from host via os.Getenv
  +-- Return Pod struct
  |
  v
...
The `Runner` interface is the central design decision. It abstracts Docker CLI operations behind four methods:
...
newSession()
  |
  +-- goroutine 1 (container): runFn(pw) -> exitCode, exitErr -> pw.Close()
  |                                                                    |
  +-- goroutine 2 (event):     bufio.Scanner(pr)                      |
                                  ...
                                  +-- emit ContainerExited or Error
                                  +-- close(events)
                                  +-- close(done)
```

The pipe close is the coordination mechanism. When the container goroutine finishes, it closes `pw`. This causes `bufio.Scanner` in the event goroutine to reach EOF, triggering the terminal event sequence. No explicit signalling between goroutines is needed.

## Container Naming

Start uses unique session IDs (`<podName>-<hex8>`) as container names. This allows multiple concurrent Start calls for different pods without naming conflicts.

Resume uses deterministic names (`cldpd-<podName>`) to find existing containers. Docker itself is the state store -- if the container exists and is running, resume works.

## Credential Passthrough

Credentials reach the container via Docker CLI flags only. No temporary files, no disk writes:

- `inheritEnv` -- The Dispatcher resolves each name via `os.Getenv`, merges the value into the `Env` map. `runCmdArgs` emits `-e KEY=VALUE` flags. Names whose values are empty on the host are silently skipped.
- `mounts` -- `runCmdArgs` emits `-v source:target[:ro]` flags.
...
**Why can output events be dropped?**

The event channel has a 256-entry buffer. If the consumer falls behind, output events are dropped via `select/default` to prevent the event goroutine from blocking indefinitely. Lifecycle events (build, container start/exit, error) always block until delivered -- they are never dropped. This ensures the caller always sees the complete lifecycle even if some output lines are lost under extreme backpressure.
