...
A pod with no `pod.json` is valid. All configuration fields are optional. The zero-value configuration produces sensible defaults: the image is tagged `cldpd-<name>`, no environment variables are injected, no mounts are attached, and no working directory is overridden.
...
- **Start** -- Discovers the pod, resolves `inheritEnv` from host environment, builds the image synchronously, then returns a `*Session` with the container running in a background goroutine. If the build fails, Start returns an error and no Session is created.
