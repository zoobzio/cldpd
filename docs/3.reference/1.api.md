...
The Dispatcher resolves `inheritEnv` entries from the host environment via `os.Getenv` and merges them into the `Env` map before passing to the Runner. Empty host values are silently skipped.
...
> **Warning:** If the caller never consumes `Events()`, the channel buffer (256) will fill under high output volume. Once full, the terminal lifecycle event blocks until space is available, which means `Wait()` will also block indefinitely. Either consume `Events()` or use `Stop`/`Wait` without consuming events only when container output is known to be low-volume.
